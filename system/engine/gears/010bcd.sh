	...
	if [ -e "$CODEPATH" ]; then
		if [ "$zipalign" ]; then
			if [ "$($B basename $CODEPATH )" = "framework-res.apk" ] || [ "$($B basename $CODEPATH )" = "com.htc.resources.apk" ]; then
				...
				/system/engine/bin/zipalign -c 4 $CODEPATH
				ZIPCHECK=$?
				if [ "$ZIPCHECK" -eq 1 ]; then
					echo " ZipAligning $CODEPATH..."
					/system/engine/bin/zipalign -f 4 $CODEPATH /cache/$($B basename $CODEPATH )
					rc="$?"
					if [ "$rc" -eq 0 ]; then
...
sync
echo $line
$B mount -o remount,ro /system 2>/dev/null
$B mount -o remount,ro `$B mount | grep system | awk '{print $1,$3}' | sed -n 1p` 2>/dev/null
STOP=`$B date +%s`
ENDED=`date`
